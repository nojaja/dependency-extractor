...
describe('DirWalker', () => {
    ...
    describe('walk method', () => {
        ...
        test('should log debug information when debug mode is enabled', async () => {
            ...
            expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringContaining('シンボリックリンクをスキップ'));
            expect(mockLoggerDebug).toHaveBeenCalledWith(expect.stringContaining('ファイル発見'));
        });
        ...
        test('should handle file callback errors', async () => {
        ...
        test('should log errors when no error callback provided', async () => {
            const readError = new Error('Permission denied');
            mockReaddir.mockRejectedValue(readError);
            ...
            expect(result).toBe(0);
            expect(mockLoggerError).toHaveBeenCalledWith(expect.stringContaining('ディレクトリ読み取りエラー'));
        });

        test('should pass settings to file callback', async () => {
            const settings = { filter: '*.js', encoding: 'utf8' };
            mockReaddir.mockResolvedValue(['test.txt']);
            mockStat.mockResolvedValue({
                isDirectory: () => false,
                isSymbolicLink: () => false
